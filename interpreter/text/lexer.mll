...
rule token = parse
  ...
  | keyword as s
    { match s with
      ...
      | "br_if" -> BR_IF
      | "br_table" -> BR_TABLE
      | "br_on_null" -> BR_CAST_NULL br_on_null
      | "br_on_cast" -> BR_CAST (br_on_cast, br_on_cast_null)
      | "br_on_non_null" -> BR_CAST_FAIL_NULL br_on_non_null
      | "br_on_cast_fail" -> BR_CAST_FAIL (br_on_cast_fail, br_on_cast_fail_null)
      | "return" -> RETURN
      | "if" -> IF
      | "then" -> THEN
      | "else" -> ELSE
      ...
      | "ref.is_null" -> REF_TEST_NULL ref_is_null
      | "ref.test" -> REF_TEST (ref_test, ref_test_null)
      | "ref.as_non_null" -> REF_CAST_NULL ref_as_non_null
