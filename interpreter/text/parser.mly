...
let inline_func_type (c : context) ft loc =
  ...
  | None ->
    let i = anon_type c loc in define_type c (dt @@ loc);
    i @@ loc
...
type_ :
  | def_type { let at = $sloc in fun c -> define_type c ($1 c @@ at) }
...
literal_ref :
  ...
  | LPAR REF_EXTERN NAT RPAR { Script.ExternRef (nat32 $3 $loc($3)) }
...
result :
  ...
  | LPAR VEC_CONST VEC_SHAPE list(numpat) RPAR {
    ...
    VecResult (VecPat (Value.V128 ($3, List.map (fun lit -> lit $3) $4))) @@ $sloc
  }
