...
struct
  ...
  let rec instr e =
    ...
    | Load (x, ({ty = I32Type; pack = Some (Pack16, ZX); _} as mo)) ->
      op 0x2f; memop x mo
    | Load (x, {ty = I32Type; pack = Some ((Pack32 | Pack64), _); _}) ->
      assert false
    ...
    | Load (x, ({ty = I64Type; pack = Some (Pack32, ZX); _} as mo)) ->
      op 0x35; memop x mo
    | Load (x, {ty = I64Type; pack = Some (Pack64, _); _}) ->
      assert false
    ...
    | Store (x, ({ty = I64Type; pack = Some Pack32; _} as mo)) ->
      op 0x3e; memop x mo
    | Store (x, {ty = I64Type; pack = Some Pack64; _}) ->
      assert false
    | Store (x, {ty = F32Type | F64Type; pack = Some _; _}) ->
      assert false

    | LoadVec (x, ({ty = V128Type; pack = None; _} as mo)) ->
      vecop 0x00l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack8x8, SX)); _} as mo)) ->
      vecop 0x01l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack8x8, ZX)); _} as mo)) ->
      vecop 0x02l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack16x4, SX)); _} as mo)) ->
      vecop 0x03l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack16x4, ZX)); _} as mo)) ->
      vecop 0x04l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack32x2, SX)); _} as mo)) ->
      vecop 0x05l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack32x2, ZX)); _} as mo)) ->
      vecop 0x06l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack8, ExtSplat); _} as mo)) ->
      vecop 0x07l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack16, ExtSplat); _} as mo)) ->
      vecop 0x08l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack32, ExtSplat); _} as mo)) ->
      vecop 0x09l; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtSplat); _} as mo)) ->
      vecop 0x0al; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack32, ExtZero); _} as mo)) ->
      vecop 0x5cl; memop x mo
    | LoadVec (x, ({ty = V128Type; pack = Some (Pack64, ExtZero); _} as mo)) ->
      vecop 0x5dl; memop x mo
    | LoadVec _ -> assert false

    | LoadVecLane (x, ({ty = V128Type; pack = Pack8; _} as mo), i) ->
      vecop 0x54l; memop x mo; u8 i;
    | LoadVecLane (x, ({ty = V128Type; pack = Pack16; _} as mo), i) ->
      vecop 0x55l; memop x mo; u8 i;
    | LoadVecLane (x, ({ty = V128Type; pack = Pack32; _} as mo), i) ->
      vecop 0x56l; memop x mo; u8 i;
    | LoadVecLane (x, ({ty = V128Type; pack = Pack64; _} as mo), i) ->
      vecop 0x57l; memop x mo; u8 i;

    | StoreVec (x, ({ty = V128Type; _} as mo)) ->
      vecop 0x0bl; memop x mo

    | StoreVecLane (x, ({ty = V128Type; pack = Pack8; _} as mo), i) ->
      vecop 0x58l; memop x mo; u8 i;
    | StoreVecLane (x, ({ty = V128Type; pack = Pack16; _} as mo), i) ->
      vecop 0x59l; memop x mo; u8 i;
    | StoreVecLane (x, ({ty = V128Type; pack = Pack32; _} as mo), i) ->
      vecop 0x5al; memop x mo; u8 i;
    | StoreVecLane (x, ({ty = V128Type; pack = Pack64; _} as mo), i) ->
