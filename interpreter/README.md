...
It provides a global object named `WebAssemblyText`. Currently, the only method on this object is `encode`, which turns a module in S-expression syntax into a WebAssembly binary. For example:
```
...
(new WebAssembly.Instance(new WebAssembly.Module(binary))).exports.f(3, 4)  // 7
```
