...
let rec step (c : config) : config =
  ...
  let vs', es' =
    ...
    | Plain e', vs ->
      ...
      | Load (x, {offset; ty; sz; _}), Num (I32 i) :: vs' ->
      ...
      | Store (x, {offset; sz; _}), Num n :: Num (I32 i) :: vs' ->
        let mem = memory frame.inst x in
        let a = I64_convert.extend_i32_u i in
        (try
          (match sz with
      ...
      | MemoryFill x, Num (I32 n) :: Num k :: Num (I32 i) :: vs' ->
        if mem_oob frame x i n then
          ...
          vs', List.map (at e.at) [
            ...
            Plain (Const (k @@ e.at));
            Plain (Store
              (x, {ty = I32Type; align = 0; offset = 0l; sz = Some Pack8}));
            Plain (Const (I32 (I32.add i 1l) @@ e.at));
            Plain (Const (k @@ e.at));
            Plain (Const (I32 (I32.sub n 1l) @@ e.at));
            Plain (MemoryFill x);
      ...
      | MemoryCopy (x, y), Num (I32 n) :: Num (I32 s) :: Num (I32 d) :: vs' ->
        if mem_oob frame x d n || mem_oob frame y s n then
          vs', [Trapping (memory_error e.at Memory.Bounds) @@ e.at]
        else if n = 0l then
          vs', []
        else if I32.le_u d s then
          vs', List.map (at e.at) [
            ...
            Plain (Const (I32 s @@ e.at));
            Plain (Load
              (y, {ty = I32Type; align = 0; offset = 0l; sz = Some (Pack8, ZX)}));
            Plain (Store
              (x, {ty = I32Type; align = 0; offset = 0l; sz = Some Pack8}));
            Plain (Const (I32 (I32.add d 1l) @@ e.at));
            Plain (Const (I32 (I32.add s 1l) @@ e.at));
      ...
      | MemoryInit (x, y), Num (I32 n) :: Num (I32 s) :: Num (I32 d) :: vs' ->
        if mem_oob frame x d n || data_oob frame y s n then
          vs', [Trapping (memory_error e.at Memory.Bounds) @@ e.at]
        else if n = 0l then
          vs', []
        else
          let seg = !(data frame.inst y) in
