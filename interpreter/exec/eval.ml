...
let type_ (inst : module_inst) x = lookup "type" (force inst.types) x
let func (inst : module_inst) x = lookup "function" (force inst.funcs) x
let table (inst : module_inst) x = lookup "table" (force inst.tables) x
let memory (inst : module_inst) x = lookup "memory" (force inst.memories) x
let global (inst : module_inst) x = lookup "global" (force inst.globals) x
...
let create_func (inst : module_inst) (f : func) : func_inst =
  Func.alloc (type_ inst f.it.ftype) inst f
...
let init_elem (inst : module_inst) (seg : table_segment) =
...
let init_data (inst : module_inst) (seg : memory_segment) =
  let {index; offset = const; init} = seg.it in
...
let check_import (m : module_) (ext : extern) (im : import) =
...
let init (m : module_) (exts : extern list) : module_inst =
  ...
  if List.length exts <> List.length imports then
    Link.error m.at "wrong number of imports provided";
  List.iter2 (check_import m) exts imports;
  let rec inst =
    { types = lazy (List.map (fun type_ -> type_.it) types);
      funcs = lazy (extern_funcs exts @ List.map (create_func inst) funcs);
      tables = lazy (extern_tables exts @ List.map (create_table inst) tables);
      memories = lazy (extern_memories exts @ List.map (create_memory inst) memories);
      globals =
        lazy (extern_globals exts @ List.map (create_global inst) globals);
      exports = lazy (List.map (create_export inst) exports);
    }
  in
  let init_elems = List.map (init_elem inst) elems in
  let init_datas = List.map (init_data inst) data in
