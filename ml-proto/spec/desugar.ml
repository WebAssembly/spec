...
let rec expr e = let e', es' = expr' e.at e.it in es' @ [e' @@ e.at]
and expr' at = function
  ...
  | Ast.Br (x, eo) -> Break (arity eo, x), expr_opt eo
  | Ast.Br_if (x, eo, e) -> BreakIf (arity eo, x), expr_opt eo @ expr e
  | Ast.Br_table (xs, x, eo, e) ->
    BreakTable (arity eo, xs, x), expr_opt eo @ expr e
  | Ast.Return eo -> Return (arity eo), expr_opt eo
  | Ast.If (e, es1, es2) ->
    If ([Block (expr_list es1) @@ at], [Block (expr_list es2) @@ at]), expr e
  | Ast.Select (e1, e2, e3) -> Select, expr e1 @ expr e2 @ expr e3

  | Ast.Call (x, es) -> Call x, expr_list es
  | Ast.Call_import (x, es) -> CallImport x, expr_list es
  | Ast.Call_indirect (x, e, es) -> CallIndirect x, expr_list es @ expr e
