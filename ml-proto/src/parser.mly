...
module_fields :
  ...
  | LPAR EXPORT var_list RPAR module_fields
    { fun c -> let m = $5 c in {m with exports = $3 c func @ m.exports} }
...
modul :
  ...
  | func  /* Sugar */
    { let memory = (Int64.zero, Int64.zero) in
      {memory; data = ""; funcs = [$1 c0]; exports = [0 @@ at()]; globals = [];
       tables = []} @@ at() }
;
...
cmd :
  | modul { Define $1 @@ at() }
  | LPAR INVOKE INT expr_list RPAR { Invoke (int_of_string $3, $4 c0) @@ at() }
  | LPAR INVOKE expr_list RPAR { Invoke (0, $3 c0) @@ at() }  /* Sugar */
  | LPAR ASSERTEQ LPAR INVOKE INT expr_list RPAR expr_list RPAR 
    { AssertEqInvoke (int_of_string $5, $6 c0, $8 c0) @@ at() }
  | LPAR ASSERTEQ LPAR INVOKE expr_list RPAR expr_list RPAR 
    { AssertEqInvoke (0, $5 c0, $7 c0) @@ at() }  /* Sugar */
;
cmd_list :
  | /* empty */ { [] }
  | cmd cmd_list { $1 :: $2 }
