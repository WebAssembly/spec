...
let rec extract_typ_atoms typ =
  ...
  | El.Ast.AtomT atom -> (match atom.it with
    | Il.Atom.Atom id -> [ id ]
    | _ -> [])
  | El.Ast.IterT (typ_inner, _) -> extract_typ_atoms typ_inner.it
  ...
  | El.Ast.CaseT (_, typs, typcases, _) ->
      ...
      ids @ typcases
  | El.Ast.SeqT tl -> List.concat_map (fun t -> extract_typ_atoms t.it) tl
  | _ -> []
...
let extract_syntax_atoms' def =
  ...
  | El.Ast.TypD (id, subid, _, typ, _) ->
      ...
      in
      let variants = extract_typ_atoms typ.it in
