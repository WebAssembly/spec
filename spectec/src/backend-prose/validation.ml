...
(* Data Structures *)
module Env = struct
  module ValueTypeEnvKey = struct
    type t = name

    let compare a b =
      Stdlib.compare (Print.string_of_name a) (Print.string_of_name b)
  end

  module TypeEnvKey = struct
    type t = string
  ...
  let find key (value_env, _) =
    try ValueTypeEnv.find key value_env
    with Not_found ->
...
let rec typeof env expr =
  ...
  | IndexAccessE (e1, e2) -> (
      ...
      let ty2 = typeof env e2 in
      subtype ty2 IntT;
      match ty1 with ListT ty -> ty | _ -> failwith "Unreachable")
...
let rec valid_instr env instr =
  ...
  | PushI e ->
      ...
      env
  | PopI (ConstE (_, NameE name)) -> Env.add name IntT env
  | PopI (NameE n) -> Env.add n WasmValueTopT env
  | PopI (IterE (name, _)) -> Env.add name (ListT WasmValueTopT) env
...
and valid_instrs env instrs = List.fold_left valid_instr env instrs
...
let init_env params =
  List.fold_left
    (fun acc (e, ty) ->
      match e with NameE n -> Env.add n ty acc | _ -> acc (* TODO *))
    Env.empty params
  |> Env.add (N "z") StateT
