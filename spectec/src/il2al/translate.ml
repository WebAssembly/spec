...
and exp2expr exp =
  ...
  (* CaseE *)
  | Il.CaseE (Il.Atom cons, argexp) -> caseE (name2kwd cons exp.note, exp2exprs argexp) ~at:at
  (* Tuple *)
  | Il.TupE exps -> tupE (List.map exp2expr exps) ~at:at
  (* Call *)
  | Il.CallE (id, args) -> callE (id.it, args2exprs args) ~at:at
  (* Record expression *)
  | Il.StrE expfields ->
      let f acc = function
        | Il.Atom name, fieldexp ->
...
let rec rhs2instrs exp =
  ...
  | Il.MixE
      ...
      | Il.CallE (f, args) -> push_instrs @ [ performI (f.it, args2exprs args) ~at:at ]
