...
.. math::
   ...
   \mbox{(data type)} & {\mathit{datatype}} &::=& \mathsf{ok} \\[0.8ex]
   {} \\[-2ex]
...
**Typing** :math:`\boxed{{\mathit{context}} \vdash {\mathit{instr}} : {\mathit{functype}}}`

An instruction sequence :math:`{{\mathit{instr}}^\ast}` is well-typed with an instruction type :math:`{{\mathit{t}}_{{1}}^\ast} \rightarrow {{\mathit{t}}_{{2}}^\ast}`, written :math:`{{\mathit{instr}}^\ast} : {{\mathit{t}}_{{1}}^\ast} \rightarrow {{\mathit{t}}_{{2}}^\ast}`, according to the following rules:
...
**Reduction** :math:`\boxed{{{{\mathit{instr}}}^\ast} \hookrightarrow {{{\mathit{instr}}}^\ast}}`
