...
relation Instr_ok: `%|-%:%`(context, instr, functype)
  ...
  `%|-%:%`(C, @SELECT(?()), `%->%`(([t] :: ([t] :: [@I32])), [t]))
    -- Valtype_sub: `|-%<:%`(t, t')
    -- iff ((t' = (numtype : numtype <: valtype)) \/ (t' = (vectype : vectype <: valtype)))
...
def default : valtype -> val
  ...
  def {reftype : reftype} default(reftype : reftype <: valtype) = @REF.NULL(reftype)
...
relation Step_pure: `%~>%`(admininstr*, admininstr*)
  ...
  `%~>%`([@LABEL(n, instr'*, ((val : val <: admininstr)* :: ([@BR((l + 1))] :: (instr : instr <: admininstr)*)))], ((val : val <: admininstr)* :: [@BR(l)]))

  ;; reduction.spasm:50.1-51.67
  rule br-zero {instr : instr, instr' : instr, n : nat, val : val, val' : val}:
  `%~>%`([@LABEL(n, instr'*, ((val' : val <: admininstr)* :: ((val : val <: admininstr)^n :: ([@BR(0)] :: (instr : instr <: admininstr)*))))], ((val : val <: admininstr)^n :: (instr' : instr <: admininstr)*))
  ...
  `%~>%`(((val : val <: admininstr)^k :: [@LOOP(bt, instr*)]), [@LABEL(n, [@LOOP(bt, instr*)], ((val : val <: admininstr)^k :: (instr : instr <: admininstr)*))])
    -- iff (bt = `%->%`(t_1^k, t_2^n))

  ;; reduction.spasm:33.1-35.29
  rule block {bt : blocktype, instr : instr, k : nat, n : nat, t_1 : valtype, t_2 : valtype, val : val}:
  `%~>%`(((val : val <: admininstr)^k :: [@BLOCK(bt, instr*)]), [@LABEL(n, [], ((val : val <: admininstr)^k :: (instr : instr <: admininstr)*))])
    -- iff (bt = `%->%`(t_1^k, t_2^n))

  ;; reduction.spasm:29.1-31.15
  rule select-false {c : c_numtype, t : valtype, val_1 : val, val_2 : val}:
  `%~>%`(([(val_1 : val <: admininstr)] :: ([(val_2 : val <: admininstr)] :: [@CONST(@I32, c) @SELECT(t?)])), [(val_2 : val <: admininstr)])
    -- iff (c = 0)

  ;; reduction.spasm:25.1-27.17
  rule select-true {c : c_numtype, t : valtype, val_1 : val, val_2 : val}:
  `%~>%`(([(val_1 : val <: admininstr)] :: ([(val_2 : val <: admininstr)] :: [@CONST(@I32, c) @SELECT(t?)])), [(val_1 : val <: admininstr)])
    -- iff (c =/= 0)

  ;; reduction.spasm:22.1-23.24
  rule drop {val : val}:
  `%~>%`(([(val : val <: admininstr)] :: [@DROP]), [])
...
relation Step_read: `%~>%`(config, admininstr*)
  ...
  `%~>%`(`%;%`(z, ((val : val <: admininstr)^k :: [@CALL_ADDR(a)])), [@FRAME(n, `%;%`(m, (val^k :: $default(t)*)), [@LABEL(n, [], (instr : instr <: admininstr)*)])])
...
relation Step: `%~>%`(config, config)
  ...
  `%~>%`(`%;%`(z, (instr : instr <: admininstr)*), `%;%`(z, (instr' : instr <: admininstr)*))
    -- Step_read: `%~>%`(`%;%`(z, (instr : instr <: admininstr)*), (instr' : instr <: admininstr)*)

  ;; reduction.spasm:7.1-9.34
  rule pure {instr : instr, instr' : instr, z : state}:
  `%~>%`(`%;%`(z, (instr : instr <: admininstr)*), `%;%`(z, (instr' : instr <: admininstr)*))
    -- Step_pure: `%~>%`((instr : instr <: admininstr)*, (instr' : instr <: admininstr)*)
