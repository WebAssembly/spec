...
function uniqueTest(func, desc) { test(func, testNum() + desc); }
  ...
  chain = chain.then(_ => {
    let spectest = {
      ...
      table : new WebAssembly.Table(
          {initial : 10, maximum : 20, element : 'anyfunc'}),
...
function module(bytes, valid = true) {
  ...
  uniqueTest(_ => { assert_equals(valid, validated); }, test);

  chain =
      chain.then(_ => WebAssembly.compile(buffer))
          .then(
              module => {
                uniqueTest(_ => { assert_true(valid, loc); }, test);
                return module;
              },
              error => {
                uniqueTest(_ => {
                  assert_true(
                      !valid,
                      `WebAssembly.compile failed unexpectedly with ${
                                                                      error
                                                                    } at {loc}`);
                }, test);
              });
...
function assert_invalid(bytes) { module(bytes, EXPECT_INVALID); }
          ...
          .then(
              ...
              error => {
                uniqueTest(
                    _ => {assert_true(
                        !valid,
                        `unexpected instantiation error, observed ${error} ${
                                                                             loc
                                                                           }`)},
                    test);
                return error;
...
function exports(name, instance) {
  return instance.then(inst => { return {[name] : inst.exports}; });
}

function call(instance, name, args) {
  return Promise.all([ instance, chain ])
      .then(values => {return values[0].exports[name](...args)});
};
...
function run(action) {
  ...
  const loc = new Error().stack.toString().replace("Error", "");
  chain =
      Promise.all([ chain, action() ])
          .then(_ => {uniqueTest(_ => {}, test)},
                error => {
                  uniqueTest(_ => {
                    assert_true(
                        false,
...
function assert_exhaustion(action) {
  ...
  const loc = new Error().stack.toString().replace("Error", "");
  chain =
      Promise.all([ action(), chain ])
          .then(_ => { uniqueTest(_ => { assert_true(false, loc); }, test); },
                error => {
                  uniqueTest(_ => {
                    assert_true(
...
function get(instance, name) {
  ...
  chain =
      Promise.all([ instance, chain ])
          .then(values => { return values[0].exports[name]; },
                _ => { uniqueTest(_ => { assert_true(false, loc); }, test); });
  return chain;
}
